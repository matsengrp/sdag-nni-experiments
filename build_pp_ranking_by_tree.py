...
def build_dag_with_all_adj_nnis(name, fasta_path, newick_path, mmap_path):
    ...
    for nni in adj_nnis:
        dag.add_node_pair(nni.parent(), nni.child())
    return dag
...
print("# building PCSP posterior map over adj NNIs...")
dag_with_all_nnis = build_dag_with_all_adj_nnis(
    "seed_dag", fasta_path, seed_trees_path, mmap_path1)
mb_pcsp_pp_map = build_pcsp_pp_map_over_dag_pcsps(
    dag_with_all_nnis, mb_tree_map, mb_tree_pp_map)

# print("# building PCSP posterior map over all NNIs in support...")
# support_inst = build_dag_inst("support_dag", fasta_path, mb_trees_path)
# dag_with_entire_support = support_inst.get_dag()
# mb_support_pcsp_pp_map = build_pcsp_pp_map_over_dag_pcsps(
#     dag_with_entire_support, mb_tree_map, mb_tree_pp_map)

# find total posterior
print("# computing total pp...")
total_pp = 0
total_trees = 0
for tree_id in mb_tree_pp_map:
    total_pp += mb_tree_pp_map[tree_id]
    total_trees += 1
print("# total_pp: ", total_pp, "total_trees: ", total_trees)

# get posterior probability of base dag
print("# finding seed dag pp...")
base_dag_pp, base_dag_tree_count = accumulate_pp_over_trees(
    dag, mb_tree_map, mb_tree_pp_map)
print("# base_dag_pp: ", base_dag_pp,
      "base_dag_tree_count: ", base_dag_tree_count)
...
for nni_id, nni in enumerate(adj_nnis):
    ...
    nni_inst = build_dag_inst(fasta_path, seed_trees_path, mmap_path3)
    nni_dag = nni_inst.get_dag()
    nni_dag.add_node_pair(nni.parent(), nni.child())
...
for nni_id in nni_dag_pp_map:
    ...
    out = f"{nni_id} {nni.parent().to_subsplit()} {nni.child().to_subsplit()} {pp} {tree_count}"
    print(out)
    fp.write(out + "\n")
fp.close()
print("# ...done")
